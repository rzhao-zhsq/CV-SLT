...
class RecognitionNetwork(torch.nn.Module):
    def __init__(
        ...
        if 'pretrained_path_rgb' in cfg:
            ...
            if self.visual_backbone is not None and self.visual_backbone_twostream is None:
    ...
    def forward(
        ...
        if self.input_type == 'video':
            ...
            if self.input_streams == ['rgb']:
                ...
                head_outputs['ensemble_last_gloss_probabilities'] = head_outputs['ensemble_last_gloss_logits'].softmax(
                    2)
                valid_len_out = sgn_lengths
        else:
            raise ValueError
                    ...
                    if soft_or_hard == 'soft':
                        for stream, gls_prob_log_lst in aux_prob_log.items():
                            for student_log_prob in gls_prob_log_lst:
                                assert teacher_prob.shape == student_log_prob.shape, (
                                    teacher_prob.shape, student_log_prob.shape)
                                outputs[f'{teaching_type}_loss'] += loss_func(input=student_log_prob,
                                                                              target=teacher_prob)
                    else:
                        for stream, gls_logits in aux_logits.items():
                            for student_logits in gls_logits:
                                B, T, V = student_logits.shape
                                outputs[f'{teaching_type}_loss'] += loss_func(input=student_logits.view(-1, V),
                                                                              target=teacher_prob.view(-1)) / B
                else:
                    if soft_or_hard == 'soft':
